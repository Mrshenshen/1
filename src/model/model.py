...
class TextImgPersonReidNet(nn.Module):
    def __init__(self, opt):
        ...
        self.text_embed = model_class.from_pretrained('SRCF-main/bert/')
        ...
        conv_local3 = []
        for i in range(opt.part):
            conv_local3.append(conv(1024, opt.feature_length))
        self.conv_local3 = nn.Sequential(*conv_local3)

        self.conv_global = conv(2048, opt.feature_length)
        self.conv_global3 = conv(1024, opt.feature_length)
        ...
        self.f204821024 = nn.Linear(2048, 1024)
    ...
    def forward(self, image, caption_id, text_length,epoch=None):
        if self.training:
            img_global, img_local, img_non_local,img_part_response,img_global_response, img, \
            image_global3, image_local3, image_non_local3,part_response3,global_mutual3, imgf3 = self.img_embedding(image,epoch=epoch)
            txt_global, txt_local, txt_non_local,txt_part_response,txt_global_response, txt, \
            text_global1024, text_local1024, text_non_local1024,part_response1024,global_mutual1024, text_feature_l1024 = self.txt_embedding(caption_id, text_length,epoch=epoch)
        else:
            img_global, img_local, img_non_local, image_global3, image_local3, image_non_local3 = self.img_embedding(image,epoch=epoch)
            txt_global, txt_local, txt_non_local, text_global1024, text_local1024, text_non_local1024 = self.txt_embedding(caption_id, text_length,epoch=epoch)
        if self.training:
            return img_global, img_local, img_non_local, txt_global, txt_local, txt_non_local,\
                   ...
                   img, txt, image_global3, image_local3, image_non_local3,text_global1024, text_local1024, text_non_local1024,\
                   part_response3,part_response1024,global_mutual3, global_mutual1024, imgf3, text_feature_l1024
        else:
            return img_global, img_local, img_non_local, txt_global, txt_local, txt_non_local,\
    ...
    def img_embedding(self, image,epoch=None):
        ...
        image_feature3_global = self.global_avgpool(imgf3)  # b,2048,1
        image_global = self.conv_global(image_feature_global).unsqueeze(2)  # b,1024
        image_global3 = self.conv_global3(image_feature3_global).unsqueeze(2)  # b,1024
        if self.training:
            ...
            image_feature_local3,part_response3,global_mutual3 = self.compute_global_local(torch.cat([imgf3],dim=0),image=True,train=self.training,epoch=epoch)
        else:
            image_feature_local,part_response,global_mutual = self.compute_global_local(image_feature,image=True,train=self.training)
            image_feature_local3,part_response3,global_mutual3 = self.compute_global_local(imgf3,image=True,train=self.training)

        image_feature_local = torch.cat(image_feature_local,dim=2)
        image_feature_local3 = torch.cat(image_feature_local3,dim=2)
        image_local = []
        image_local3 = []
        for i in range(self.opt.part):
        ...
        for i in range(self.opt.part):
            image_feature_local3_i = image_feature_local3[:, :, i, :]
            image_feature_local3_i = image_feature_local3_i.unsqueeze(2)
            image_embedding_local3_i = self.conv_local3[i](image_feature_local3_i).unsqueeze(2)
            image_local3.append(image_embedding_local3_i)

        image_local3 = torch.cat(image_local3, 2)#b,1024,6

        image_non_local3 = self.leaky_relu(image_local3)
        image_non_local3 = self.non_local_net(image_non_local3)

        if self.training:
            return image_global, image_local, image_non_local,part_response,global_mutual, image_feature, image_global3, image_local3, image_non_local3,part_response3,global_mutual3, imgf3
        else:
            return image_global, image_local, image_non_local, image_global3, image_local3, image_non_local3
    ...
    def txt_embedding(self, caption_id, text_length,epoch=None):
        ...
        f1024, fword_1 = self.model_txt(txt)
        text_feature_l = fword_1.squeeze().unsqueeze(-1)
        text_feature_l1024 = f1024.squeeze().unsqueeze(-1)
        ...
        text_global1024 = self.global_avgpool(text_feature_l1024)  # 64,2048
        text_global1024 = self.conv_global(text_global1024).unsqueeze(2)  # 64,1024
        if self.training:
            ...
            text_feature_local1024,part_response1024,global_mutual1024 = self.compute_global_local(text_feature_l1024.squeeze().permute(2,0,1),image=False,txt_lang=text_length,train=self.training,epoch=epoch)
        else:
            ...
            text_feature_local1024,_ = self.compute_global_local(text_feature_l1024.squeeze().permute(2,0,1),image=False,txt_lang=text_length,train=self.training,epoch=epoch)

        text_feature_local = torch.cat(text_feature_local, dim=-1)#b,2048,6
        text_feature_local1024 = torch.cat(text_feature_local1024, dim=-1)#b,2048,6
        ...
        text_local1024 = []
        for p in range(self.opt.part):
            text_feature_local_conv_p1024 = text_feature_local1024[:, :, p].unsqueeze(2).unsqueeze(2)
            text_feature_local_conv_p1024 = self.conv_local3[p](text_feature_local_conv_p1024).unsqueeze(2)
            text_local1024.append(text_feature_local_conv_p1024)
        text_local1024 = torch.cat(text_local1024, dim=2)#b,1024,6

        text_non_local = self.leaky_relu(text_local)
        text_non_local = self.non_local_net(text_non_local)#b,512,6

        text_non_local1024 = self.leaky_relu(text_local1024)
        text_non_local1024 = self.non_local_net(text_non_local1024)#b,512,6
        if self.training:
            return text_global, text_local, text_non_local,part_response,global_mutual, text_feature_l, text_global1024, text_local1024, text_non_local1024,part_response1024,global_mutual1024, text_feature_l1024
        else:
            return text_global, text_local, text_non_local, text_global1024, text_local1024, text_non_local1024
